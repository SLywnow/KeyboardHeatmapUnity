...
namespace SLywnow
{
	...
	public class SaveSystemAlt : MonoBehaviour
	{
		...
		public static string GetString(string key, string def = null, bool fromanytype = false)
		{
			...
			if (ss_sl.name.Contains(key))
				if (ss_sl.type[ss_sl.name.IndexOf(key)] == SaveSystemSL.SSLTpe.stringS || ss_sl.type[ss_sl.name.IndexOf(key)] == SaveSystemSL.SSLTpe.undefinedS || fromanytype)
				{
					return ss_sl.contain[ss_sl.name.IndexOf(key)];
				}
				else
				{
					if (useDebug) Debug.LogError("Type of value not string"); return def;
				}
			else
			{
				if (useDebug) Debug.LogError("There is no such key in SSA file with " + sindex_sl + " index"); return def;
			}
		}
	...
	public class JSONFile
	{
		...
		public int GetKeysCount(params string[] path_)
		{
			JSONFile m = this;
			for (int i = 0; i < path_.Length; i++)
			{
